...
if [[ ${function_mode} -eq 0 ]]; then
    ...
    if [[ ${if_hierarchical} -eq 0 ]]; then
        python -u ./external/MC2/Method/greedy.py \
            ...
            --output_dir ./Checkpoints/${output_dir_postfix}/greedy_${locam_minimum_threshold}_${if_feedback}_${model_name}_${eval_model_name}_if_multiple_llm_${if_multiple_llm}_if_use_vague_cg_hyp_as_input_${if_use_vague_cg_hyp_as_input}_if_generate_with_past_failed_hyp_${if_generate_with_past_failed_hyp}_bkgid_${bkg_id}_init_hyp_id_${init_hyp_id}_${output_dir_postfix}.json \
            --if_save 1 \
