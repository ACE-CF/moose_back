...
def load_MC_start_file_research_background(job_name, bkg_id=0):
    # Load the JSON file
    start_file_path = f"./StartFiles/{job_name}/{job_name}_bkg_id_{bkg_id}_custom_MC_research_background.json"
...
def load_MC_gene_hypothesis(output_dir_postfix, model_name, init_id, bkg_id=0, if_with_gdth_hyp_annotation=0):
    # Load the JSON file
...
def load_MC_matched_scores(output_dir_postfix, model_name, init_id, bkg_id=0, if_with_gdth_hyp_annotation=1):
...
# OUTPUT:
#   feedback: text
def provide_feedback(question, gdth_hyp, gene_hyp, api_type, api_key, base_url, model_name):
    ...
    prompts = demo_instruction_prompts("feedback_with_reference_stronger")
...
def demo_instruction_prompts(module_name):
    # very soft feedback
    if module_name == "feedback_with_reference_soft":
        prompts = ['''
    ...
    elif module_name == "feedback_with_reference_stronger":
        prompts = ['''
